...
export async function fetchIssuerJwks(kind: IssuerKind): Promise<{ keys: JWK[] }> {
  // Hits the public-host path (proxied to apps/issuer behind basePath="/issuer").
  const baseHost = process.env.PUBLIC_HOSTNAME?.replace(/\/$/, "") ?? "http://localhost:3000";
  const url = `${baseHost}/issuer/api/issuer/${kind}/.well-known/jwks.json`;
  const res = await fetch(url, { cache: "no-store", headers: { "ngrok-skip-browser-warning": "1" } });
  if (!res.ok) {
