    ...
    <div className="min-h-screen bg-white-50">
      ...
      <MessagesView
        ...
        threads={conversations.map((c) => {
          ...
          return {
            id: c.id,
            counterpartyUserId: c.booking?.lawyerProfile.user.id ?? "",
            ...
            // Pubkey shipped through so the client-side viewer can decrypt the
            // last-message preview even for messages this user sent (NaCl box
            // requires the OTHER party's pubkey to recover plaintext).
            counterpartyEncryptionPublicKey: c.booking?.lawyerProfile.user.encryptionPublicKey ?? null,
            subject: c.booking?.practiceArea ?? "Consultation",
            engagementChainId: c.booking?.engagement?.engagementIdOnChain ?? null,
            caseSummary: c.booking?.caseDescription ?? null,
            lastMessage: last
              ? {
                  ...
                  nonce: last.nonce,
                  senderEncryptionPublicKey: last.senderEncryptionPublicKey,
                }
              : null,
            lastTime: (last?.createdAt ?? c.updatedAt).toISOString(),
