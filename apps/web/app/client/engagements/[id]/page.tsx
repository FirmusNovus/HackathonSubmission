...
/**
 * Client-side per-engagement summary. Mirrors /lawyer/engagements/[id] but
 * links the booking row to /client/cases/[bookingId] and the follow-up
 * orders to /client/follow-ups/[orderId].
 */
export default async function ClientEngagementDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!engagement || engagement.clientId !== session.user.id) notFound();

  const lawyerName = engagement.lawyerProfile.user.name ?? "Your lawyer";
  ...
  return (
    <div className="min-h-screen bg-white-50">
      ...
      <main className="mx-auto max-w-[960px] px-6 py-10 lg:px-8">
        ...
        >
          <ArrowLeft className="h-3.5 w-3.5" aria-hidden /> Cases
        </Link>

        <div className="mt-6 flex flex-wrap items-baseline gap-3">
          <h1 className="font-display text-3xl text-navy-900">Engagement with {lawyerName}</h1>
        ...
        <section className="mt-6 rounded-2xl border border-slate-100 bg-white-0 p-6">
          ...
          {engagement.booking ? (
            <ConsultationRow
              href={`/client/cases/${engagement.booking.id}`}
        ...
        <section className="mt-6 rounded-2xl border border-slate-100 bg-white-0 p-6">
          ...
          {engagement.orders.length === 0 ? (
            <p className="mt-4 text-[13px] text-slate-500">{lawyerName} hasn't sent any follow-up orders yet.</p>
          ) : (
            <ul className="mt-3 divide-y divide-slate-100">
              {engagement.orders.map((o) => (
                <OrderListRow
                  key={o.id}
                  href={`/client/follow-ups/${o.id}`}
