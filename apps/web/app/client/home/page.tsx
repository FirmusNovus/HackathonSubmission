...
import { BookingStatus, VerificationStatus } from "@/lib/db/enums";
...
const CATEGORY_SERVICES: Record<string, Array<{ title: string; price: number; desc: string }>> = {
  Business: [
    { title: "Incorporation in your EU country", price: 0.15, desc: "Entity formation, tax registration, bank account setup" },
    { title: "Founder agreement", price: 0.12, desc: "Vesting, IP assignment, role definitions" },
    { title: "Stock option plan setup", price: 0.3, desc: "ESOP design + paperwork for 1–10 employees" },
  ],
  Immigration: [
    { title: "Schengen visa application", price: 0.1125, desc: "Document review, application, follow-up" },
    { title: "EU residency permit", price: 0.3, desc: "Full filing + follow-up consultation" },
    { title: "Family reunification", price: 0.6, desc: "End-to-end with translations included" },
  ],
  Family: [
    { title: "Pre-nuptial agreement review", price: 0.08, desc: "Cross-jurisdiction; one revision included" },
    { title: "Custody arrangement", price: 0.06, desc: "60-min strategy session for separated parents" },
    { title: "Will update", price: 0.045, desc: "30-min review + redraft of an existing will" },
  ],
  Property: [
    { title: "Residential purchase review", price: 0.095, desc: "Title check + sale-and-purchase review" },
    { title: "Commercial lease", price: 0.135, desc: "Up to 30 pages; cross-jurisdiction friendly" },
    { title: "Property dispute strategy", price: 0.07, desc: "60-min session, action plan included" },
  ],
  Employment: [
    { title: "Wrongful dismissal assessment", price: 0, desc: "Free 30-min review of your situation" },
    { title: "Executive contract review", price: 0.12, desc: "Severance, non-compete, IP — full review" },
  ],
  Tax: [
    { title: "Cross-border tax check", price: 0.09, desc: "Personal or single-entity, 1 hr" },
    { title: "Annual tax filing review", price: 0.06, desc: "Spot-check + recommended optimizations" },
  ],
  Estate: [
    { title: "EU cross-border inheritance", price: 0.12, desc: "60-min strategy + summary deliverable" },
    { title: "Estate plan setup", price: 0.275, desc: "Will + trust + tax-efficient structure" },
  ],
  All: [],
};
...
export default async function ClientHomePage({
  ...
  const recommendedAll = await prisma.lawyerProfile.findMany({
    where: { verificationStatus: VerificationStatus.VERIFIED },
    include: { user: true },
    orderBy: [{ rating: "desc" }],
    // Pull more than we need so the JS-side tag filter still has candidates.
    take: cat === "All" ? 6 : 60,
  });
  const recommended =
    cat === "All"
      ? recommendedAll
      : recommendedAll.filter((l) => parseStringArray(l.tags).includes(cat)).slice(0, 6);
              ...
              <div key={s.title} className="rounded-xl border border-slate-100 bg-white-0 p-5 shadow-[var(--shadow-sm)]">
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="font-display text-2xl text-navy-900">
                    {s.price === 0 ? "Free" : `from ${formatETH(s.price)}`}
                  </span>
