...
export default async function LawyerOrdersListPage() {
  ...
  return (
    <div className="min-h-screen bg-white-50">
      ...
      <main className="mx-auto max-w-[1080px] px-6 py-10 lg:px-8">
        <h1 className="font-display text-3xl text-navy-900 sm:text-4xl">Pending orders</h1>
        <p className="mt-2 text-[15px] text-slate-500">Review and respond to consultation orders from prospective clients.</p>

        {orders.length === 0 ? (
          <div className="mt-8">
            <EmptyState
              title="All caught up."
              body="New consultation orders show here. You'll also get a dot on the dashboard."
          ...
          <ul className="mt-8 divide-y divide-slate-100 rounded-xl border border-slate-100 bg-white-0">
            {orders.map((r) => (
              <li key={r.id}>
                <Link href={`/lawyer/orders/${r.id}`} className="flex items-start justify-between gap-4 p-5 hover:bg-white-50">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1.5 text-[12px] text-slate-500">
                      {formatScheduled(r.scheduledAt)} · {r.durationMinutes} min · {formatETH(Number(r.consultationFeeEUR))}
                    </div>
                  </div>
