...
export default async function LawyerFollowUpDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!order || order.engagement.lawyerProfile.userId !== session.user.id) notFound();
  ...
  return (
    <div className="min-h-screen bg-white-50">
      ...
      <main className="mx-auto max-w-[760px] px-6 py-10 lg:px-8">
        <Link
          href={`/lawyer/engagements/${order.engagement.id}`}
        ...
        <div className="mt-6 flex flex-wrap items-baseline gap-3">
          <h1 className="font-display text-3xl text-navy-900">Follow-up order</h1>
        ...
        <section className="mt-7 rounded-2xl border border-slate-100 bg-white-0 p-6">
          <Row label="Client" value={order.engagement.client.name ?? truncateAddress(order.engagement.client.walletAddress)} />
        ...
        <section className="mt-6 rounded-2xl border border-slate-100 bg-white-0 p-6">
          <div className="text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">Description</div>
        ...
        <div className="mt-6 flex justify-end gap-2.5">
          <OrderFollowUpActions
            ...
            counterpartyName={order.engagement.client.name ?? "the client"}
            perspective="lawyer"
          />
        </div>
      </main>
