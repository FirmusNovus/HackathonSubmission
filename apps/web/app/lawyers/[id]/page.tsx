...
import { parseStringArray, parseJsonArray } from "@/lib/db/json-fields";
...
export default async function LawyerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const items = parseJsonArray<PricingItem>(lawyer.pricingItems);
  const tags = parseStringArray(lawyer.tags);
  const languages = parseStringArray(lawyer.languages);
  const jurisdictions = parseStringArray(lawyer.jurisdictions);
  const specialties = parseStringArray(lawyer.specialties);
                ...
                {isHourly ? (
                  <>
                    ...
                    <div className="flex items-baseline gap-2">
                      <span className="font-display text-4xl text-navy-900">{formatETH(Number(lawyer.hourlyRateEUR))}</span>
                      <span className="text-[14px] text-slate-500">per hour · ETH</span>
                    </div>
                  </>
                ) : (
                  <>
                        ...
                        <li key={i} className="rounded-lg border border-slate-100 bg-white-50 px-3.5 py-3">
                          <div className="flex items-baseline justify-between gap-3">
                            ...
                            <span className="whitespace-nowrap text-[13px] font-medium text-navy-900">
                              {it.price === null ? it.unit : it.price === 0 ? "Free" : formatETH(it.price)}
