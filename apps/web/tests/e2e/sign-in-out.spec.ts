...
test.describe("Sign-in / sign-out top-right control", () => {
  // TODO(phase-2): rewrite once the connect+SIWE redesign lands. The marketing
  // nav no longer exposes a literal "Sign In" link — the WalletButton replaces it.
  test.skip("Disconnected viewer sees a Sign In CTA, no separate sign-in link", async ({ page }) => {
    await page.goto("/", { waitUntil: "domcontentloaded" });
  ...
  // TODO(phase-2): the post-signout assertion looks for a "Sign In" link that's
  // gone in the redesign. Re-enable after the SIWE rewrite settles the new pattern.
  test.skip("Account menu signs the user out and clears the session cookie", async ({ page, context }) => {
    await devSignIn(page, { wallet: SEEDED.client1, role: "client" });
