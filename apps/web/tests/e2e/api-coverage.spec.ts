...
test.describe("Authenticated endpoints — lawyer role", () => {
  ...
  test("POST /api/bookings/[id]/accept — records lawyer signature, status stays REQUESTED", async ({ page, browser }) => {
    ...
    expect(r.status()).toBe(200);
    // Phase 6: accept just records the lawyer signature. Status only flips
    // to ACCEPTED after the client submits the on-chain funding tx and
    // /api/bookings/[id]/funded confirms the receipt.
    const data = (await r.json()) as { booking: { status: string; lawyerAcceptedAt: string | null } };
    expect(data.booking.status).toBe("REQUESTED");
    expect(data.booking.lawyerAcceptedAt).not.toBeNull();
    await ctx2.close();
  });

  // TODO(phase-9): decline route gets reshaped (mutual-refund flow); 403 path
  // will move to the new endpoint.
  test.skip("POST /api/bookings/[id]/decline — when not the assigned lawyer, 403", async ({ page }) => {
