...
import { issueAccessToken } from '@firmus-novus/oid4vci';
...
export async function POST(req: NextRequest) {
  ...
  if (!preAuthCode) return NextResponse.json({ error: 'invalid_grant' }, { status: 400 });
  const issued = issueAccessToken(getDb(), preAuthCode, body.tx_code);
  if (!issued) return NextResponse.json({ error: 'invalid_grant' }, { status: 400 });
