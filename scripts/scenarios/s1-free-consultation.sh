...
# Carlos García (persona 2) offers FREE 30-min consultations.
# A client books, Carlos accepts, client marks complete.
...
CARLOS=$(lawyer_addr carlos-garcia)
expect_match "$CARLOS" '^0x[a-fA-F0-9]{40}$' "Carlos address resolved"
...
RES=$(curl -s -X POST -H "Content-Type: application/json" -b "$client_cookie" \
  --data "{\"lawyerAddress\":\"$CARLOS\",\"scheduledAt\":$(date -d 'tomorrow 14:00' +%s),\"durationMinutes\":30,\"practiceArea\":\"Property\",\"caseDescription\":\"Need a free intro on cross-border property purchase in Spain.\"}" \
...
# Carlos accepts.
carlos_cookie=$(mktemp)
login_as 2 "$carlos_cookie"
ACC=$(curl -s -X POST -b "$carlos_cookie" "$PLATFORM/api/consultations/$CID/accept")
expect_match "$ACC" "ok.*true" "carlos accept ok"
