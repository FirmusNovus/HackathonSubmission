...
describe("dcql", () => {
  ...
  it("buildPidDcql requests given_name, family_name, age_equal_or_over.18, address.country", () => {
    const q = buildPidDcql();
    const paths = q.credentials[0].claims.map((c) => c.path.join("."));
    expect(paths).toEqual(["given_name", "family_name", "age_equal_or_over.18", "address.country"]);
  });

  it("buildPidDcql still excludes nationalities, birth_date, document_number, place_of_birth, sex", () => {
    const q = buildPidDcql();
    const flat = q.credentials[0].claims.flatMap((c) => c.path).join(",");
