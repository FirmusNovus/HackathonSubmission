...
import type { Driver } from "./drivers/interface.ts";

import * as codec from "./codecs/jwk_jcs-pub.ts";
import { KEY_DID_METHOD_PREFIX } from "./constants.ts";
import * as jwkDriver from "./drivers/jwk_jcs-pub.ts";
import { InvalidDidError } from "./errors/index.ts";
...
export function resolveDidDoc(did: string, contentType?: string): DIDDocument {
  ...
  if (!driver) {
    throw new InvalidDidError(`Unsupported codec ${code.toString()}`);
  }
