...
export function findSubjectByAddress(address: Address): PidSubjectRow | null {
  const row = getDb()
    .prepare(`SELECT ${COLUMNS} FROM pid_subjects WHERE lower(eth_address) = lower(?)`)
...
export function findSubjectById(id: number): PidSubjectRow | null {
  const row = getDb()
    .prepare(`SELECT ${COLUMNS} FROM pid_subjects WHERE id = ?`)
    .get(id) as StoredRow | undefined;
  return row ? inflate(row) : null;
}

export interface PidPersonaRow extends PidSubjectRow {
  eth_address: string;
}

export function listAllPid(): PidPersonaRow[] {
  const rows = getDb()
    .prepare(`SELECT ${COLUMNS}, eth_address FROM pid_subjects ORDER BY id`)
    .all() as Array<StoredRow & { eth_address: string }>;
  return rows.map((r) => ({ ...inflate(r), eth_address: r.eth_address }));
}
