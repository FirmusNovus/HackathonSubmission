...
contract LegalEngagementEscrow is ReentrancyGuard, EIP712 {
    ...
    // Production semantics: 30 days from `markDelivered` before the lawyer can
    // escalate. This preserves the asymmetric-dispute design (Inv 6, Constitution
    // principle III) — clients can dispute immediately, lawyers must wait so they
    // can't grab funds from an unhappy client. For the v3 demo we set this to
    // zero so judges don't have to fast-forward time. Restore to `30 days`
    // before any non-demo deployment.
    uint64 public constant LAWYER_DISPUTE_COOLDOWN = 0;
    ...
    /// @notice Lawyer-side escalation. Production accepts only Delivered
    ///         milestones after the LAWYER_DISPUTE_COOLDOWN has elapsed since
    ///         `markDelivered` (Inv 6). For the v3 demo we accept Funded too,
    ///         so the lawyer can escalate without first having to call
    ///         markDelivered. The role gate (onlyEngagementLawyer) and the
    ///         cooldown (now zero) are still here — re-tightening them for
    ///         production is a one-line change in each.
        ...
        onlyEngagementLawyer(engagementId)
    {
        ...
        if (m.state == MilestoneState.Delivered) {
