...
interface AppTopBarProps {
  ...
  active?: "home" | "messages" | "cases" | "dashboard" | "requests" | "profile";
  dark?: boolean;
}
        ...
        {links.map((it) => {
          ...
          return (
            <Link
              ...
              className={
                dark
                  ? `text-[13px] font-medium ${isActive ? "text-white" : "text-white/70 hover:text-white"}`
                  : `text-[13px] font-medium ${isActive ? "text-navy-900" : "text-slate-500 hover:text-navy-900"}`
              }
            >
              {it.label}
            </Link>
